...
public class NoticeUserDetailServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(result > 0) { // 유효한 공지사항 번호 => 상세 조회
			...
			request.getRequestDispatcher("views/notice/noticeUserDetailView.jsp").forward(request, response);
			...
			response.sendRedirect(request.getContextPath() + "/list.uno?currentPage=1");
